...
def cost_data_combine(aws_node_cost, gcp_node_cost, azure_node_cost, gcp_store_cost, aws_store_cost, StorageType, Number_of_Storage_Units):
    ...
    # sorting
    gcp_cost = gcp_node_cost.sort_values(
        by=['Monthly Cost with Storage']).drop(columns=['hourly'])
    aws_cost = aws_node_cost.sort_values(
        ['Monthly Cost with Storage']).drop(columns=['hourly'])
    azure_cost = azure_node_cost.sort_values(
        by=['Monthly Cost with Storage']).drop(columns=['hourly'])
    # comining cost
